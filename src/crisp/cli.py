...
def cmd_plots(args) -> None:
    ...
    paths = plots.build(args.results_dir or report.RESULTS_DIR,
                        args.out or plots.FIGURES_DIR)
    for path in paths:
        print(path)
    if not paths:
        print("no results to plot yet; run scripts/reproduce.sh first")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_plots = sub.add_parser("plots", help="render figures from artifacts/results")
    p_plots.add_argument("--results-dir")
    p_plots.add_argument("--out", help="figure directory (default: artifacts/figures)")
